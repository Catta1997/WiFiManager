...
// Funzione di utilità per ripetere un carattere 'count' volte
...
void printBox(String username, String password, String ip1) {
  // Prepara le righe con le etichette desiderate
  String line1 = "Username: " + username;
  String line2 = "Password: " + password;
  String line3 = "IP: " + ip1;
  
  // Calcola la lunghezza massima tra le righe
  int maxLen = max(max(line1.length(), line2.length()), line3.length());
  Serial.println("");
  Serial.println("");
  // Stampa il bordo superiore
  Serial.println("+" + repeatChar('-', maxLen + 2) + "+");
  
  // Funzione lambda per stampare una riga con il padding necessario
  auto printLine = [&](String line) {
    int padding = maxLen - line.length();
    Serial.println("| " + line + repeatChar(' ', padding) + " |");
  };
...
void WiFiManager::startWebPortal() {
  ...
  printBox("admin", pass, WiFi.localIP().toString());
  #endif
