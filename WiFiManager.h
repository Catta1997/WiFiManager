...
#define WiFiManager_h
	...
	// #include "soc/efuse_reg.h" // include to add efuse chip rev to info, getChipRevision() is almost always the same though, so not sure why it
	// matters.
	...
	// #include <esp_idf_version.h>
		...
		// esp_get_idf_version
			...
			// #pragma message "ESP_ARDUINO_VERSION_PATCH = " WM_STRING(ESP_ARDUINO_VERSION_PATCH)
				...
				#define VER_ARDUINO_STR \
					WM_STRING(ESP_ARDUINO_VERSION_MAJOR) "." WM_STRING(ESP_ARDUINO_VERSION_MINOR) "." WM_STRING(ESP_ARDUINO_VERSION_PATCH)
			#else
...
class WiFiManager {
   ...
   protected:
	...
	uint8_t _connectRetries =
		1;  // number of sta connect retries, force reconnect, wait loop (connectimeout) does not always work and first disconnect bails
	bool _aggresiveReconn = false;  // use an agrressive reconnect strategy, WILL delay conxs
	                                // on some conn failure modes will add delays and many retries to work around esp and ap bugs, ie, anti de-auth
	                                // protections https://github.com/tzapu/WiFiManager/issues/1067
   ...
   protected:
	...
	boolean _disableIpFields =
		false;  // modify function of setShow_X_Fields(false), forces ip fields off instead of default show if set, eg. _staShowStaticFields=-1
